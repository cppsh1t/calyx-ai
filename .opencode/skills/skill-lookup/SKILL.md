---
name: skill-lookup
description: Activates when the user asks about Agent Skills, wants to find reusable AI capabilities, needs to install skills, or mentions skills for opencode. Use for discovering, retrieving, and installing skills.
---

When the user needs Agent Skills, wants to extend opencode's capabilities, or is looking for reusable AI agent components, use the prompts.chat MCP server.
...
- Mentions extending opencode's capabilities with skills
...
2. Create the directory `.opencode/skills/{slug}/`
3. Save each file to the appropriate location:
   - `SKILL.md` → `.opencode/skills/{slug}/SKILL.md`
   - Other files → `.opencode/skills/{slug}/{filename}`
