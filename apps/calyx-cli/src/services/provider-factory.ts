...
/**
 * Stream text using any supported provider
 * @param providerId - The provider identifier (e.g., 'deepseek', 'anthropic')
 * @param modelId - The model identifier (e.g., 'deepseek-reasoner', 'gpt-4')
 * @param message - The prompt/message to send
 * @param options - Optional additional parameters for streamText
 * @returns StreamTextResult with textStream property for iteration
 * @throws Error if provider is not supported or API key is missing
 */
export async function streamWithProvider(providerId: string, modelId: string, message: string, options?: Record<string, unknown>) {
  // Get or create provider factory
  const provider = await getProviderFactory(providerId)

  // Create model instance
  const model = (provider as { languageModel: (modelId: string) => LanguageModel }).languageModel(modelId)

  // Stream text with the model
  return streamText({
    model,
    prompt: message,
    ...options,
