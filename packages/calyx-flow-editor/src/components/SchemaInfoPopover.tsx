...
export function SchemaInfoPopover({ title, schema, description, className }: SchemaInfoPopoverProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={[
          'pointer-events-none absolute z-40 mt-2 min-w-52 max-w-72 rounded-lg border border-slate-200/80 bg-white/95 p-3 text-left shadow-xl backdrop-blur-sm',
          'left-0 top-full opacity-0 transition duration-150 group-hover/info:opacity-100 group-focus-within/info:opacity-100',
          isOpen ? 'opacity-100' : '',
        ].join(' ')}
      >
